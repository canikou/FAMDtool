...
class ReportTests(unittest.TestCase):
    def test_empty_week_export_matches_manual_format(self):
        with TemporaryDirectory() as temp_dir:
            ...
            text = build_weekly_export_text(db, date(2026, 6, 8))
    ...
    def test_export_uses_only_selected_week_logs_and_shifts(self):
        with TemporaryDirectory() as temp_dir:
            ...
            db.add_shift(datetime(2026, 6, 15, 5, 0), datetime(2026, 6, 15, 6, 0))
            db.add_log("response", date(2026, 6, 7), "old", "ROBBERY", "Yeol", "", "")
            db.add_log("response", date(2026, 6, 8), "1000", "ROBBERY", "Yeol", "", "")
            db.add_log("response", date(2026, 6, 15), "future", "ROBBERY", "Yeol", "", "")

            text = build_weekly_export_text(db, date(2026, 6, 8))

            self.assertIn("Time-in: 3:00AM", text)
            self.assertIn("Total Robbery/Distress Responses: 1", text)
            self.assertIn("TOTAL HOURS FOR THE WEEK: 1h00m", text)
            self.assertNotIn("1:00AM", text)
